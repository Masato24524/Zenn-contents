...
EC2などを起動したままにしておくと課金が継続されるため、不要なAWSリソースは停止しておきましょう。（できれば削除しておくのがベストです）

特にEC2、Elastic IP（VPCから確認可能）、　は注意してください。

### さいごに

今回得られた知見
- IAMロールはSSM側ではなく、EC2側（アクセスされる側）に設定する

- CloudFormationの便利さ。一発で環境を作れるのは凄い（IAMロールだって作れる）

- Firehose →　S3へと直接データを吐き出せる（間に別のリソースは不要）
